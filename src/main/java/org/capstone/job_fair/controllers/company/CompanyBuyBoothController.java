...
public class CompanyBuyBoothController {
    ...
    public ResponseEntity<?> companyBuyBooth(@Valid @RequestBody CompanyBuyBoothRequest request) {
        ...
        JobFairBoothDTO jobFairBoothDTO = companyBoothPurchaseService.purchaseBooth(request.getCompanyRegistrationId(), request.getBoothId());

        return ResponseEntity.ok(jobFairBoothDTO);
    }
}
